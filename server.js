...
// Server-side fetch to the upstream API. The upstream host, path and key
// NEVER reach the client — Burp/inspect sees only our own /api endpoints.
...
async function getCredentials(gameId) {
  const raw = await apiGet('sentgame.php', { gameid: gameId, key: API_KEY });
  const data = JSON.parse(raw);
  return { username: data.Username || data.username || '', password: data.Password || data.password || '' };
}
...
app.get('/api/credentials', async (req, res) => {
  ...
  if (!gid || !/^[A-Za-z0-9_-]{1,32}$/.test(gid)) return res.status(400).json({ error: 'Invalid request.' });
  try {
...
app.listen(port, '0.0.0.0', () => console.log(`BESTTHING listening on ${port}`));
